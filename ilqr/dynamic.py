...
class AutoDiffDynamics(Dynamics):
    ...
    def __init__(self, f, x_inputs, u_inputs, i=None, hessians=False, **kwargs):
        ...
        """
        self._tensor = f
        self._t = T.dscalar("i") if i is None else i
...
class BatchAutoDiffDynamics(Dynamics):  # Batch in time dimension
    ...
    def __init__(self, model, state_size, action_size, **kwargs):
        """Constructs a BatchAutoDiffDynamics model.

        Args:
            model: Symbolic function with the following signature:
                Args:
        ...
        """
        self._fn = model
        self._state_size = state_size  # Dim of input = 5
        self._action_size = action_size  # Dim of control = 1
        ...
        self._tensor = model(x_rep_1, u_rep_1, i_rep_1)
        self._f = as_function(self._tensor, inputs, name="f", **kwargs)

        self._J_x, self._J_u, _ = batch_jacobian(model, inputs, state_size)
