...
grad = NeuralODE(CartPoleDynamic().to(device), sensitivity='adjoint', solver='euler')
dynamics = GeneralDyanmic(0.02, grad, state_size=5, action_size=1)

# env init
env = Monitor(CartPoleCustomEnv(), './gym_data/true_cartpole', force=True)
...
last_us = pred_u_constrained[0:steps]
...
for i in range(150):
    ...
    pred, pred_u = controller.fit(next_obs,
                                  np.vstack([all_pred_us[-1][steps:, :], all_pred_us[-1][-steps:, :].reshape(1, 1)]),
                                  n_iterations=20,
...
torch.save({'all_obs': all_obs,
            ...
            'all_info': all_info,
            'all_u': all_u}, 'data/true_mpc_ilqr_cartpole.pt')
